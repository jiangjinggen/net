...
static int readresp(char c, netbuf *nControl)
{
	...
	{
		//LOG_ERROR("readresp Control socket read failed");
		return 0;
	}

	if (ftplib_debug > 1)	fprintf(stderr,"%s",nControl->response);
	...
	//LOG_ERROR("readresp not Match expect[%c] ret[%c] retbuf[%s]",c,nControl->response[0],nControl->response);
	return 0;
}
...
static int FtpSendCmd(const char *cmd, char expresp, netbuf *nControl)
{
	...
	{
		//LOG_ERROR("nControl->dir != FTPLIB_CONTROL");
		return 0;
	}
	...
	{
		//LOG_ERROR("strlen(cmd[%s]) + 3) > sizeof(buf[%d])",cmd,sizeof(buf));
		return 0;
	}
	...
	{
		//LOG_ERROR("net_write Fail[%s]",buf);
		return 0;
	}
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
	...
	if (nControl->dir != FTPLIB_CONTROL)
	{
		//LOG_ERROR("FtpOpenPort nControl->dir != FTPLIB_CONTROL");
		return -1;
	}
	
	if ((dir != FTPLIB_READ) && (dir != FTPLIB_WRITE))
	{
		//LOG_ERROR("FtpOpenPort Invalid direction [%d]", dir);
	...
	{
		//LOG_ERROR("FtpOpenPort Invalid mode [%d]", mode);
	...
	if (nControl->cmode == FTPLIB_PASSIVE)
	{
		...
		{
			//LOG_ERROR("FtpOpenPort FtpSendCmd PASV fail");
	    return -1;
	  }
		...
		if (cp == NULL)
		{
			//LOG_ERROR("FtpOpenPort cp = strchr(nControl->response,'(') == NULL");
	    return -1;
	  }
	...
	}else
	{
		...
		{
			//LOG_ERROR("FtpOpenPort getsockname Fail");
			return 0;
		}
	}
	...
	if (sData == -1)
	{
		//LOG_ERROR("FtpOpenPort socket Fail[%d]",sData);
		return -1;
	}

	if (setsockopt(sData,SOL_SOCKET,SO_REUSEADDR,SETSOCKOPT_OPTVAL_TYPE &on,sizeof(on)) == -1)
	{
		//LOG_ERROR("FtpOpenPort setsockopt SO_REUSEADDR Fail");
		net_close(sData);
		return -1;
	}
	...
	{
		//LOG_ERROR("FtpOpenPort setsockopt SO_LINGER Fail");
		net_close(sData);
		return -1;
	}
	...
	if (nControl->cmode == FTPLIB_PASSIVE)
	{
		...
		{
			//LOG_ERROR("FtpOpenPort connect Fail");
	    net_close(sData);
	    return -1;
		}
	}else
	{
		...
		{
			//LOG_ERROR("FtpOpenPort bind Fail");
	    net_close(sData);
	    return 0;
		}
		if (listen(sData, 1) < 0)
		{
			//LOG_ERROR("FtpOpenPort listen Fail");
	    net_close(sData);
	    return 0;
		}
		...
		{
			//LOG_ERROR("FtpOpenPort FtpSendCmd 2 Fail");
	    net_close(sData);
	    return 0;
		}
	}
	...
	if (ctrl == NULL)
	{
		//LOG_ERROR("FtpOpenPort calloc Fail");
		net_close(sData);
		return -1;
	}
	...
	{
		//LOG_ERROR("FtpOpenPort (mode == 'A') && ((ctrl->buf = malloc(FTPLIB_BUFSIZ)) == NULL)");
		net_close(sData);
		free(ctrl);
		return -1;
	}
...
GLOBALDEF int FtpAccess(const char *path, int typ, int mode, netbuf *nControl, netbuf **nData)
{
	...
	{
		//LOG_ERROR("FtpAccess Missing path argument for file transfer");
	...
	if (!FtpSendCmd(buf, '2', nControl))
	{
		//LOG_ERROR("FtpAccess FtpSendCmd 2[%s] Fail",buf);
		return 0;
	}
	...
	case FTPLIB_FILE_WRITE:
		...
		break;
	default:
		//LOG_ERROR("FtpAccess Invalid open type %d[%d] Fail",typ);
	...
	{
		//LOG_ERROR("FtpAccess FtpOpenPort[%d][%d] Fail",mode,dir);
		return 0;
	}
	
	ret = FtpSendCmd(buf, '1', nControl);
	if (ret==0)
	{
		//LOG_ERROR("FtpAccess FtpSendCmd 1[%s] Fail[%d]",buf,ret);
		FtpClose(*nData);
		*nData = NULL;
		return 0;
	}
	...
	if (nControl->cmode == FTPLIB_PORT)
	{
		...
		{
			//LOG_ERROR("FtpAccess FtpAcceptConnection Fail");
			FtpClose(*nData);
			*nData = NULL;
			nControl->data = NULL;
			return 0;
...
static int FtpXfer(const char *localfile, const char *path,	netbuf *nControl, int typ, int mode)
{
	...
	if (localfile != NULL)
	{
		...
		if (local == NULL)
		{
			//LOG_ERROR("FtpXfer fopen localfile[%s] fail errno[%d][%s]",errno,strerror(errno));
			strncpy(nControl->response, strerror(errno), sizeof(nControl->response));
			return 0;
		}
	}
	...
	{
		//LOG_ERROR("FtpXfer FtpAccess path[%s] fail",path);
		return 0;
	}
	...
	if (typ == FTPLIB_FILE_WRITE)
	{
	    ...
	    {
				//LOG_ERROR("short write: passed %d, wrote %d", l, c);
				rv = 0;
				break;
	    }
	}else
